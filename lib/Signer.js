...
class Signer {
  ...
  _constructCanonicalRequestForAPI(access_token, params, encoded_query_string){
    ...
    canonical.push('x-amz-date:' + this._iso_date.full);
    if (params.amzShippingBizId) {
      canonical.push('x-amzn-shipping-business-id:' + req_params.amzShippingBizId);
    }
    canonical.push('');
    canonical.push('host;user-agent;x-amz-access-token;x-amz-date' + params.amzShippingBizId ? ';x-amzn-shipping-business-id' : '');
  ...
  signAPIRequest(access_token, role_credentials, req_params){
    ...
    return {
      ...
      headers:{
        ...
        ...(req_params.amzShippingBizId ? {'x-amzn-shipping-business-id': req_params.amzShippingBizId} : {}),
